...
from src.nn.scoreNNBlock import SinEmbedding, FourierEmbedding
from src.dataLib.data_util import scatter_center
...
class CSPVLayer(nn.Module):
    ...
    def edge_model(
        ...
        vi, vj = vt[edge_index[0]], vt[edge_index[1]]
...
class CSPVNet(nn.Module):
    ...
    def forward(
        ...
        edge_graph_index = batch[edge_index[0]]
        ...
        if self.pred_v:
            ...
            if self.zero_cog:
                out_v = scatter_center(out_v, batch)

            out["v"] = out_v

        if self.pred_l:
            graph_features = scatter(node_features, batch, dim=0, reduce="mean")
